...
def build_system_prompt(has_context: bool = False) -> str:
    ...
    base = (
        ...
        "طريقة العمل:\n"
        "• كن ودوداً وطبيعياً في الحوار —像个 محامي صديق بيشرح القانون.\n"
        "• اجب من النصوص المسترجعة من قاعدة المعرفة، واذكر رقم المادة عند الاستشهاد.\n"
        "• إذا لم يكن هناك سياق، قل ببساطة: 'لا توجد مادة قانونية مطابقة داخل قاعدة المعرفة الحالية.'\n"
...
def generate_answer(query: str, context_chunks: list[dict[str, Any]], history: list[dict[str, str]] | None = None) -> str:
    ...
    if context_text:
        ...
        user_message += (
            "لا توجد مواد قانونية متاحة. "
            "رد فقط: 'لا توجد مادة قانونية مطابقة داخل قاعدة المعرفة الحالية.'\n"
        )
